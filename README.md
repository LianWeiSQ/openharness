...
OpenAgent 是一个 Python Agent Core Runtime，用来构建能调用工具、执行代码、运行在隔离环境里、管理长上下文并留下可复盘 trace 的 coding agent / task agent。
...
- **Isolated execution**: the same loop can run against a local workspace, a remote sandbox runtime, Terminal-Bench, or Harbor.
...
OpenAgent includes an agent loop, built-in tools, permission rulesets, context budgeting, structured compaction, file/instruction context, local/remote sandbox runtimes, Terminal-Bench and Harbor adapters, MCP support, and JSONL-friendly observability.
...
| 执行环境 | 本地 workspace、远端 sandbox runtime、Terminal-Bench、Harbor |
...
| --- | --- |
| `local` | 在本地仓库执行工具 |
| `opensandbox` | 可选的远端 sandbox runtime 适配 |
...
- 完善 remote sandbox runtime adapter 和 skill bootstrap

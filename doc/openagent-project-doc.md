...
- 本地工作区与 OpenSandbox 工作区 runtime 抽象

当前仍未完整落地的部分：

- Anthropic、Gemini、Ollama Provider 仍是 stub
- `legacy_cli.py` 入口缺失，但测试仍引用它
...
│   │   ├── execution/         # local / opensandbox 工作区 runtime
...
- `src/tests/test_legacy_cli.py` 引用 `legacy_cli.py`，当前仓库没有该文件。
- `src/tests/test_execution_runtime.py` 中 OpenSandbox fake search 的测试形态与 runtime 当前调用方式不一致。
...
- OpenSandbox 模式下只暴露 `workspace` 和 `agnostic` 工具，不暴露 `host_only` 工具。
...
- OpenSandbox 模式要求 `sandbox_id` 和绝对 POSIX `remote_workdir`。
...
4. `ToolkitAdapter.register_mcp()` 桥接到内部工具系统。

## 17. 文档维护规则

- README 只保留入口、运行方式、已知限制和文档索引。
- 本文档描述当前代码事实，不写长期愿景。
- 设计细节放在专题文档，例如 OpenSandbox、Web 研究收敛。

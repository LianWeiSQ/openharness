...
> 面向：Agent 架构设计复盘、技术公开说明、后续 Roadmap 对齐
...
CLI / Web Console 不属于当前公开 core 测试范围；公开 core 测试以 `src/tests` 下的 unittest 为准。
...
OpenAgent 将上下文压缩从自由文本 summary 升级成结构化 work state，并加入预算感知投影，让长任务在压缩、恢复和低上下文预算下都能稳定继续。
...
这就是从“摘要”变成了“可恢复工作现场”。公开设计文档只保留工程事实和可复用原则，不包含个人准备材料、特定组织语境或非公开方案。

...
| CLI-03 | MCP management commands | `packages/opencode/src/cli/cmd/mcp.ts` provides add/list/auth/logout/debug flows and remote OAuth support. | Partial | OpenAgent now supports `openagent mcp list/show/add/remove/doctor`, `mcp auth list/status/set-token`, `mcp logout`, and `mcp debug` for the current remote MCP config/header/token model, with JSON/table output, redacted headers/secrets, default config resolution, chmod 0600 writes, and structured debug/doctor refresh failures. Full browser OAuth flow and dynamic client registration remain tracked in [#69](https://github.com/LianWeiSQ/openagent-ai/issues/69). | P0 | [#43](https://github.com/LianWeiSQ/openagent-ai/issues/43), [#65](https://github.com/LianWeiSQ/openagent-ai/issues/65), [#69](https://github.com/LianWeiSQ/openagent-ai/issues/69) | `PYTHONPATH=src:src/tests python -m unittest src/tests/test_mcp_config.py src/tests/test_mcp_runtime.py src/tests/test_openagent_cli.py`; `PYTHONPATH=src python -m openagent.cli.main mcp auth --help`; `PYTHONPATH=src python -m openagent.cli.main mcp debug --help` | Config-management slice implemented in [#43](https://github.com/LianWeiSQ/openagent-ai/issues/43). Supported-equivalent auth/logout/debug CLI slice implemented in [#65](https://github.com/LianWeiSQ/openagent-ai/issues/65); row remains Partial until full OAuth browser flow/dynamic registration lands in [#69](https://github.com/LianWeiSQ/openagent-ai/issues/69). |
| CLI-04 | Provider-aware credentials | `packages/opencode/src/cli/cmd/providers.ts` implements provider login/list/logout, methods, and well-known provider behavior. | Partial | OpenAgent now supports provider-aware `auth` plus `providers` alias login/list/logout, normalized provider ids, provider-specific env metadata, redacted multi-provider credential records, env-only provider discovery, `providers methods [provider]`, active-provider env/model visibility, and native Anthropic provider routing that does not depend on an OpenAI-compatible `/models` endpoint. Full OpenCode parity still needs security-reviewed well-known provider URL login in [#70](https://github.com/LianWeiSQ/openagent-ai/issues/70) plus remaining non-Anthropic/provider-catalog native gaps in [#71](https://github.com/LianWeiSQ/openagent-ai/issues/71). | P0 | [#44](https://github.com/LianWeiSQ/openagent-ai/issues/44), [#67](https://github.com/LianWeiSQ/openagent-ai/issues/67), [#68](https://github.com/LianWeiSQ/openagent-ai/issues/68), [#70](https://github.com/LianWeiSQ/openagent-ai/issues/70), [#71](https://github.com/LianWeiSQ/openagent-ai/issues/71) | `PYTHONPATH=src:src/tests python -m unittest src/tests/test_openagent_cli.py src/tests/test_openai_streaming.py src/tests/test_anthropic_provider.py src/tests/test_app_server_runtime.py`, `PYTHONPATH=src python -m openagent.cli.main providers methods anthropic --format json`, `OPENAGENT_PROVIDER=anthropic ANTHROPIC_API_KEY=test PYTHONPATH=src python -m openagent.cli.main models --format json`, and `OPENAGENT_PROVIDER=anthropic ANTHROPIC_API_KEY=test PYTHONPATH=src python -m openagent.cli.main doctor --format json` (expects healthy only when the optional `anthropic` package is installed; dependency-missing environments must return non-healthy JSON without leaking the key). | Provider-aware credential layer implemented in [#44](https://github.com/LianWeiSQ/openagent-ai/issues/44). Env-only discovery, auth method metadata, and active provider runtime/model visibility implemented in [#67](https://github.com/LianWeiSQ/openagent-ai/issues/67). Anthropic native routing slice for [#68](https://github.com/LianWeiSQ/openagent-ai/issues/68) adds a no-network-tested native adapter/factory, provider-aware doctor JSON with dependency checks, and native model listing; row remains Partial until [#70](https://github.com/LianWeiSQ/openagent-ai/issues/70) and [#71](https://github.com/LianWeiSQ/openagent-ai/issues/71) land. |
...
| TUI-10 | Interactive App Bridge attach | `cli/cmd/tui/attach.ts`, `run.ts --interactive --attach`, and server TUI-control routes support attaching a TUI to an existing server. | Supported | OpenAgent supports `openagent attach <url>` for a local curses TUI backed by App Bridge REST/SSE sessions, turns, interrupts, approvals, global `/api/events` replay, and the [#66](https://github.com/LianWeiSQ/openagent-ai/issues/66) `/tui/*` control slice: authenticated append/submit/clear prompt controls, help/session opens, command execution, toast display, publish mapping, session selection, control polling, and control responses. Broader TUI model/theme/palette/plugin gaps remain tracked in their own rows. | P0 | [#62](https://github.com/LianWeiSQ/openagent-ai/issues/62), [#63](https://github.com/LianWeiSQ/openagent-ai/issues/63), [#66](https://github.com/LianWeiSQ/openagent-ai/issues/66) | `PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=src:src/tests python -m unittest src/tests/test_app_server_server.py src/tests/test_app_server_runtime.py src/tests/test_tui_remote_runtime.py src/tests/test_tui_formatting.py`; `PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=src python -m py_compile src/openagent/app_server/server.py src/openagent/app_server/runtime.py src/openagent/tui/remote_runtime.py src/openagent/tui/state.py src/openagent/tui/app.py`; `git diff --check`. | [#62](https://github.com/LianWeiSQ/openagent-ai/issues/62) interactive attach vertical path implemented; [#63](https://github.com/LianWeiSQ/openagent-ai/issues/63) adds global App Bridge SSE for attached TUI event consumption; [#66](https://github.com/LianWeiSQ/openagent-ai/issues/66) adds server-side TUI control routes, remote attached TUI polling/response, local dispatch, and focused auth/body/queue/response/state tests. |
| TUI-11 | Curses TUI consumes App Bridge SSE | `stream.transport.ts`, `stream.ts`, and `session-data.ts` separate event transport from rendering. | Supported | OpenAgent exposes global App Bridge SSE at `/api/events` with stable `global_sequence` ids, `Last-Event-ID`/`last_sequence` replay, auth parity with other API/SSE routes, and remote TUI routing into matching `RemoteTurnRecord`s with turn-scoped fallback/dedupe. | P1 | [#63](https://github.com/LianWeiSQ/openagent-ai/issues/63) | `PYTHONPATH=src:src/tests python -m unittest src/tests/test_app_server_protocol.py src/tests/test_app_server_server.py src/tests/test_tui_remote_runtime.py src/tests/test_tui_formatting.py` plus `PYTHONPATH=src python -m openagent.cli.main attach --help` and `git diff --check`. | [#63](https://github.com/LianWeiSQ/openagent-ai/issues/63) merged to main; main-agent verification passed 39 App Bridge/TUI tests, 55 App runtime/CLI regression tests, `attach --help`, and diff check. |
...
1. [CLI-03 / #69](https://github.com/LianWeiSQ/openagent-ai/issues/69):
   full MCP browser OAuth flow and dynamic client registration. The
   [#43](https://github.com/LianWeiSQ/openagent-ai/issues/43) config-management
   slice and [#65](https://github.com/LianWeiSQ/openagent-ai/issues/65)
   auth/logout/debug CLI slice cover the current remote config/header/token
   model, but OpenCode's browser OAuth exchange remains a P0 CLI gap.
2. [CLI-04 / #68](https://github.com/LianWeiSQ/openagent-ai/issues/68):
   Anthropic native provider SDK routing. The broader remaining provider parity
   work is split into well-known provider URL login in
   [#70](https://github.com/LianWeiSQ/openagent-ai/issues/70) and remaining
   non-Anthropic/provider-catalog native gaps in
   [#71](https://github.com/LianWeiSQ/openagent-ai/issues/71).

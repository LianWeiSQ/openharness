...
| CLI-04 | Provider-aware credentials | `packages/opencode/src/cli/cmd/providers.ts` implements provider login/list/logout, methods, and well-known provider behavior. | Partial | OpenAgent now supports provider-aware `auth` plus `providers` alias login/list/logout, normalized provider ids, provider-specific env metadata, redacted multi-provider credential records, env-only provider discovery, `providers methods [provider]`, active-provider env/model visibility, native Anthropic provider routing, and security-reviewed supported-equivalent well-known provider URL login from [#70](https://github.com/LianWeiSQ/openagent-ai/issues/70): strict URL validation, redacted command preview, no remote command execution, user-supplied token storage, and well-known metadata storage. Full OpenCode parity still needs remaining non-Anthropic/provider-catalog native gaps in [#71](https://github.com/LianWeiSQ/openagent-ai/issues/71). | P0 | [#44](https://github.com/LianWeiSQ/openagent-ai/issues/44), [#67](https://github.com/LianWeiSQ/openagent-ai/issues/67), [#68](https://github.com/LianWeiSQ/openagent-ai/issues/68), [#70](https://github.com/LianWeiSQ/openagent-ai/issues/70), [#71](https://github.com/LianWeiSQ/openagent-ai/issues/71) | `PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=src:src/tests python -m unittest src/tests/test_openagent_cli.py`, `PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=src python -m openagent.cli.main providers login --help`, `PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=src python -m py_compile src/openagent/cli/main.py src/openagent/cli/auth.py src/openagent/cli/wellknown.py`, and `git diff --check`. | Provider-aware credential layer implemented in [#44](https://github.com/LianWeiSQ/openagent-ai/issues/44). Env-only discovery, auth method metadata, and active provider runtime/model visibility implemented in [#67](https://github.com/LianWeiSQ/openagent-ai/issues/67). Anthropic native routing slice for [#68](https://github.com/LianWeiSQ/openagent-ai/issues/68) adds a no-network-tested native adapter/factory, provider-aware doctor JSON with dependency checks, and native model listing. Security-reviewed supported-equivalent well-known provider URL login landed in [#70](https://github.com/LianWeiSQ/openagent-ai/issues/70); row remains Partial until [#71](https://github.com/LianWeiSQ/openagent-ai/issues/71) lands. |

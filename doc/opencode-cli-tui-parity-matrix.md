...
| CLI-02 | Remote App Bridge attach workflow | `packages/opencode/src/cli/cmd/tui/attach.ts` provides `attach <url>` with dir/session/continue/fork/auth options. | Partial | OpenAgent has non-interactive `client`, but no interactive TUI attach to a running App Bridge. | P1 | [#42](https://github.com/LianWeiSQ/openagent-ai/issues/42) | `PYTHONPATH=src python -m openagent.cli.main attach --help` plus an App Bridge smoke. | Pending |
| CLI-03 | MCP management commands | `packages/opencode/src/cli/cmd/mcp.ts` provides add/list/auth/logout/debug flows and remote OAuth support. | Partial | OpenAgent has MCP config/runtime tests, but no `openagent mcp ...` management CLI. | P0 | [#43](https://github.com/LianWeiSQ/openagent-ai/issues/43) | `PYTHONPATH=src:src/tests python -m unittest src/tests/test_mcp_config.py src/tests/test_mcp_runtime.py src/tests/test_openagent_cli.py` | Pending |
| CLI-04 | Provider-aware credentials | `packages/opencode/src/cli/cmd/providers.ts` implements provider login/list/logout, methods, and well-known provider behavior. | Partial | OpenAgent `auth` stores OpenAI-compatible credentials only. | P0 | [#44](https://github.com/LianWeiSQ/openagent-ai/issues/44) | `PYTHONPATH=src:src/tests python -m unittest src/tests/test_openagent_cli.py` | Pending |
...
| TUI-10 | Interactive App Bridge attach | `cli/cmd/tui/attach.ts`, `run.ts --interactive --attach`, and server TUI-control routes support attaching a TUI to an existing server. | Partial | OpenAgent has App Bridge and non-interactive `client`, but curses TUI cannot consume remote sessions/events. | P0 | [#62](https://github.com/LianWeiSQ/openagent-ai/issues/62) | Start `openagent serve --headless`, run `openagent attach <url>` smoke, and run CLI/TUI tests. | Pending |
...
1. [CLI-03 / #43](https://github.com/LianWeiSQ/openagent-ai/issues/43):
   `openagent mcp` management CLI. This unlocks existing MCP runtime/config
   work and is the clearest CLI surface gap.
2. [CLI-04 / #44](https://github.com/LianWeiSQ/openagent-ai/issues/44):
   provider-aware credentials. This underpins model listing, run, and TUI
   selection parity.
3. [TUI-10 / #62](https://github.com/LianWeiSQ/openagent-ai/issues/62):
   interactive App Bridge attach. This aligns OpenAgent with OpenCode's local
   server / attached TUI architecture and enables remote event transport work.

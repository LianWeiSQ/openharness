...
| CLI-04 | Provider-aware credentials | `packages/opencode/src/cli/cmd/providers.ts` implements provider login/list/logout, methods, and well-known provider behavior. | Partial | OpenAgent now supports provider-aware `auth` plus `providers` alias login/list/logout, normalized provider ids, provider-specific env metadata, redacted multi-provider credential records, env credential status, and active-provider env loading into the current OpenAI-compatible runtime. Full OpenCode parity still needs provider runtime/model methods, well-known provider behavior, env-only provider discovery, and non-OpenAI-compatible SDK routing in [#67](https://github.com/LianWeiSQ/openagent-ai/issues/67). | P0 | [#44](https://github.com/LianWeiSQ/openagent-ai/issues/44), [#67](https://github.com/LianWeiSQ/openagent-ai/issues/67) | `PYTHONPATH=src:src/tests python -m unittest src/tests/test_openagent_cli.py` and `PYTHONPATH=src python -m openagent.cli.main auth --help` | Provider-aware credential layer implemented in [#44](https://github.com/LianWeiSQ/openagent-ai/issues/44); 40 focused CLI tests pass locally. Row remains Partial until [#67](https://github.com/LianWeiSQ/openagent-ai/issues/67) lands full provider runtime/model-method parity. |

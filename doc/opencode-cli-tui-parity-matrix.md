...
| TUI-11 | Curses TUI consumes App Bridge SSE | `stream.transport.ts`, `stream.ts`, and `session-data.ts` separate event transport from rendering. | Partial | OpenAgent branch [#63](https://github.com/LianWeiSQ/openagent-ai/issues/63) adds global App Bridge SSE at `/api/events` with stable `global_sequence` ids, `Last-Event-ID`/`last_sequence` replay, auth parity with other API/SSE routes, and remote TUI routing into matching `RemoteTurnRecord`s with turn-scoped fallback/dedupe. Row remains Partial until main-agent merge verification lands. | P1 | [#63](https://github.com/LianWeiSQ/openagent-ai/issues/63) | `PYTHONPATH=src:src/tests python -m unittest src/tests/test_app_server_protocol.py src/tests/test_app_server_server.py src/tests/test_tui_remote_runtime.py src/tests/test_tui_formatting.py` plus `PYTHONPATH=src python -m openagent.cli.main attach --help` and `git diff --check`. | Implementation branch pending main-agent merge verification. |

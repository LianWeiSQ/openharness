...
- Decoupled swarm/function kernel with function runners, subprocess runners, HTTP runners, A2A send/stream/subscribe runners, YAML config, CLI YAML runner, local inspection API and browser view, multi-runner dispatch, an OpenAgent runner adapter, mixed OpenAgent+A2A and all-runners examples, local trace lineage, optional Langfuse export with coordinator receipt annotations, opt-in worker workspace isolation, merge-back conflict review, merge approval policy, file-backed run state receipts, resumable coordinator policy, team handoff manifests, and observability-ready coordinator workflow receipts.
...
- the swarm kernel has CLI, observability-ready receipts, a local inspection API and browser view, optional Langfuse receipt annotations, and public mixed OpenAgent/non-OpenAgent examples, but still needs more polished OpenAgent binding ergonomics for non-demo use.
...
6. Improve OpenAgent binding ergonomics for the decoupled [Swarm Function Kernel](swarm.md).

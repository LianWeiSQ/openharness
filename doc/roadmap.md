...
- Decoupled swarm/function kernel with function runners, subprocess runners, HTTP runners, A2A send/stream runners, YAML config, multi-runner dispatch, an OpenAgent runner adapter, local trace lineage, optional Langfuse export, opt-in worker workspace isolation, merge-back conflict review, merge approval policy, file-backed run state receipts, and resumable coordinator policy.
...
- the swarm kernel does not yet have resumable team adapters or A2A task subscription reconnect support.
...
6. Add resumable team adapters, A2A task subscription reconnect support, and a coordinator workflow that combines resume, merge approval, and optional apply on top of the decoupled [Swarm Function Kernel](swarm.md).

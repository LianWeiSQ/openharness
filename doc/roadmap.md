...
- Decoupled swarm/function kernel with function runners, subprocess runners, HTTP runners, YAML config, multi-runner dispatch, an OpenAgent runner adapter, local trace lineage, optional Langfuse export, opt-in worker workspace isolation, merge-back conflict review, and file-backed run state receipts.
...
- the swarm kernel does not yet have A2A, coordinator-level merge approval policy, or resumable team adapters.
...
6. Add A2A runners, resumable team adapters, and coordinator-level merge approval on top of the decoupled [Swarm Function Kernel](swarm.md).

...
- Decoupled swarm/function kernel with function runners, subprocess runners, HTTP runners, YAML config, multi-runner dispatch, an OpenAgent runner adapter, local trace lineage, optional Langfuse export, opt-in worker workspace isolation, and merge-back conflict review.
...
- the swarm kernel does not yet have A2A, coordinator-level merge approval policy, or persistent team adapters.
...
6. Add A2A runners, persistent team state, and coordinator-level merge approval on top of the decoupled [Swarm Function Kernel](swarm.md).

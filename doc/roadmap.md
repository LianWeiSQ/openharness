...
- Decoupled swarm/function kernel with function runners, subprocess runners, HTTP runners, A2A send/stream runners, YAML config, multi-runner dispatch, an OpenAgent runner adapter, local trace lineage, optional Langfuse export, opt-in worker workspace isolation, merge-back conflict review, merge approval policy, file-backed run state receipts, resumable coordinator policy, and team handoff manifests.
...
- the swarm kernel does not yet have a combined coordinator workflow or A2A task subscription reconnect support.
...
6. Add a coordinator workflow that combines resume, team handoff, merge approval, and optional apply, plus A2A task subscription reconnect support on top of the decoupled [Swarm Function Kernel](swarm.md).

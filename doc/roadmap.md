...
- Decoupled swarm/function kernel with function runners, subprocess runners, HTTP runners, A2A runners, YAML config, multi-runner dispatch, an OpenAgent runner adapter, local trace lineage, optional Langfuse export, opt-in worker workspace isolation, merge-back conflict review, merge approval policy, file-backed run state receipts, and resumable coordinator policy.
...
- the swarm kernel does not yet have resumable team adapters or streaming A2A support.
...
6. Add resumable team adapters, streaming A2A support, and a coordinator workflow that combines resume, merge approval, and optional apply on top of the decoupled [Swarm Function Kernel](swarm.md).

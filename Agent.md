...
> Adapter架构：上层暴露通用智能体接口，底层通过 Provider/SDK（或 HTTP 适配）调用不同大模型
...
└─────────────────────────────┬───────────────────────────────────┘
                              │
┌─────────────────────────────┴───────────────────────────────────┐
│                   Provider/SDK Layer (LLM Providers)             │
...
UniversalAgent ─────┬──> AgentAdapter ───> LanguageModel（Provider 适配）
                    ...
                    │
                    v
              AgentLoop ───> Session ───> Messages / Todos / MemoryAdapter
                    │
                    ├──> DoomLoopDetector
                    ├──> RetryManager
                    └──> SnapshotManager
...
- `openagent/src/examples/run_dashscope_universal.py`：UniversalAgent 问答 demo
...
- demo 默认启用 `stream=true`（SSE 真流式），并按 `tools="readonly"` 暴露只读文件工具：`read/glob/grep/ls`
- demo 默认 `permission="READONLY"`：只允许执行以上只读工具，其它工具（write/edit/bash/web/...）会被拒绝

一个最小的“工具调用”触发示例：

```bash
python openagent/src/examples/run_dashscope_universal.py "请列出当前工作目录的文件列表"
```
